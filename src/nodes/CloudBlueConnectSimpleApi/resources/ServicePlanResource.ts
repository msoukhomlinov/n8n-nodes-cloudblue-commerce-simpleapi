...
export class ServicePlanResource extends BaseResource {
  ...
  async loadOptions(
    ...
    if (propertyName === 'planId') {
      const response = await this.makeRequest<IServicePlan>(
      ...
      return response.data.map((plan: IServicePlan) => ({
  ...
  async execute(
    ...
    let result: IPaginatedResponse<unknown>;
    ...
    switch (operation) {
      case 'list': {
        ...
        if (returnAll) {
          ...
          result = await this.makeRequest<IServicePlan>(executeFunctions, 'GET', '', undefined, {
            ...filters,
            limit,
          });
        }
      ...
      case 'get': {
        ...
        result = await this.makeRequest<IServicePlan>(executeFunctions, 'GET', `/${id}`);
        break;
      }
      case 'create': {
