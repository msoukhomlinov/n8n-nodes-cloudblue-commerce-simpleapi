...
export function handleErrorWithRetry(
    ...
    if (httpError.response) {
        const status = (httpError as JsonObject & { response: { statusCode: number } }).response.statusCode;
        ...
        switch (status) {
            ...
            case 429: {
                const retryAfter = Number.parseInt(
                    (httpError as JsonObject & { response: { headers: Record<string, string> } })
                        .response.headers['retry-after'] || '60',
                    10
                );
                throw new NodeApiError(node, httpError, {
                    message: `Rate limit exceeded. Retrying in ${retryAfter} seconds...`,
                    description: `Attempt ${retryAttempt + 1} of ${maxRetries}. Will retry after ${retryAfter} seconds.`
                });
            }
            default: {
                throw new NodeApiError(node, httpError);
